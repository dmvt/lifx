(function() {
    ...
    for (y = input.min = 0; y < width; y++) {
        for (x = 0; x < width; x++) {
            ...
            window.rgbChange(rgb);
        }
    }

    // Bind Event Handlers
    input.onchange = function() { window.updateBrightness(input) };
    toggle.onchange = function() { window.updatePower(toggle) };
    window.updateBrightness(input);
    window.updatePower(toggle);
    ...
    function redraw(e) {
        ...
        label.textContent = b.style.background = resp.pop();

        window.rgbChange(resp);
